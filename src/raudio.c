...
AudioBuffer* LoadAudioBuffer(ma_format format, ma_uint32 channels, ma_uint32 sampleRate, ma_uint32 sizeInFrames, int usage) {
    ...
    ma_data_converter_config converterConfig =
        ma_data_converter_config_init(format, AUDIO_DEVICE_FORMAT, channels, AUDIO_DEVICE_CHANNELS, sampleRate, AUDIO.System.device.sampleRate);
...
Wave LoadWave(const char* fileName) {
    ...
    if (fileData != NULL)
        wave = LoadWaveFromMemory(GetFileExtension(fileName), fileData, dataSize);
...
Wave LoadWaveFromMemory(const char* fileType, const unsigned char* fileData, int dataSize) {
    ...
    if (false) {
        ...
        if (wave.data != NULL) {
            ...
            TRACELOG(LOG_WARNING, "WAVE: Failed to load QOA data");
...
bool IsWaveValid(Wave wave) {
    ...
    if ((wave.data != NULL) &&    // Validate wave data available
        ...
        (wave.channels > 0))
        result = true;  // Validate number of channels supported
...
Sound LoadSoundFromWave(Wave wave) {
    ...
    if (wave.data != NULL) {
        ...
        ma_uint32 frameCount = (ma_uint32)ma_convert_frames(NULL, 0, AUDIO_DEVICE_FORMAT, AUDIO_DEVICE_CHANNELS, AUDIO.System.device.sampleRate, NULL,
                                                            frameCountIn, formatIn, wave.channels, wave.sampleRate);
        if (frameCount == 0)
            TRACELOG(LOG_WARNING, "SOUND: Failed to get frame count for format conversion");

        AudioBuffer* audioBuffer =
            LoadAudioBuffer(AUDIO_DEVICE_FORMAT, AUDIO_DEVICE_CHANNELS, AUDIO.System.device.sampleRate, frameCount, AUDIO_BUFFER_USAGE_STATIC);
        if (audioBuffer == NULL) {
...
Sound LoadSoundAlias(Sound source) {
    ...
    if (source.stream.buffer->data != NULL) {
        AudioBuffer* audioBuffer =
            LoadAudioBuffer(AUDIO_DEVICE_FORMAT, AUDIO_DEVICE_CHANNELS, AUDIO.System.device.sampleRate, 0, AUDIO_BUFFER_USAGE_STATIC);
...
void UpdateSound(Sound sound, const void* data, int frameCount) {
    if (sound.stream.buffer != NULL) {
        ...
        memcpy(sound.stream.buffer->data, data,
               frameCount * ma_get_bytes_per_frame(sound.stream.buffer->converter.formatIn, sound.stream.buffer->converter.channelsIn));
    }
}
...
bool ExportWave(Wave wave, const char* fileName) {
    ...
    if (false) {
        ...
        if (success)
            success = (int)drwav_write_pcm_frames(&wav, wave.frameCount, wave.data);
        ...
        if (wave.sampleSize == 16) {
            ...
            TRACELOG(LOG_WARNING, "AUDIO: Wave data must be 16 bit per sample for QOA format export");
    }
#endif
...
#define TEXT_BYTES_PER_LINE 20
    ...
    if (wave.sampleSize == 32) {
        ...
        for (int i = 1; i < waveDataSize / 4; i++)
            byteCount += sprintf(txtData + byteCount, ((i % TEXT_BYTES_PER_LINE == 0) ? "%.4ff,\n    " : "%.4ff, "), ((float*)wave.data)[i - 1]);
        ...
        for (int i = 1; i < waveDataSize; i++)
            byteCount +=
                sprintf(txtData + byteCount, ((i % TEXT_BYTES_PER_LINE == 0) ? "0x%x,\n    " : "0x%x, "), ((unsigned char*)wave.data)[i - 1]);
    ...
    if (success != 0)
        TRACELOG(LOG_INFO, "FILEIO: [%s] Wave as code exported successfully", fileName);
    else
...
bool IsSoundPlaying(Sound sound) {
    ...
    if (IsAudioBufferPlaying(sound.stream.buffer))
        result = true;
...
void WaveFormat(Wave* wave, int sampleRate, int sampleSize, int channels) {
    ...
    ma_uint32 frameCount =
        (ma_uint32)ma_convert_frames(NULL, 0, formatOut, channels, sampleRate, NULL, frameCountIn, formatIn, wave->channels, wave->sampleRate);
    ...
    frameCount = (ma_uint32)ma_convert_frames(data, frameCount, formatOut, channels, sampleRate, wave->data, frameCountIn, formatIn, wave->channels,
                                              wave->sampleRate);
    if (frameCount == 0) {
...
void WaveCrop(Wave* wave, int initFrame, int finalFrame) {
    if ((initFrame >= 0) && (initFrame < finalFrame) && ((unsigned int)finalFrame <= wave->frameCount)) {
        ...
        memcpy(data, (unsigned char*)wave->data + (initFrame * wave->channels * wave->sampleSize / 8),
               frameCount * wave->channels * wave->sampleSize / 8);
...
Music LoadMusicStreamFromMemory(const char* fileType, const unsigned char* data, int dataSize) {
    ...
    if (false) {
        ...
        if (dataSize && (dataSize < 32 * 1024 * 1024)) {
            ...
            if (jar_mod_load(ctxMod, (void*)ctxMod->modfile, dataSize))
                result = dataSize;
        }
...
#endif
#if defined(SUPPORT_FILEFORMAT_QOA)
        else if (music.ctxType == MUSIC_AUDIO_QOA)
            qoaplay_close((qoaplay_desc*)music.ctxData);
#endif
#if defined(SUPPORT_FILEFORMAT_FLAC)
...
void StopMusicStream(Music music) {
    ...
    case MUSIC_AUDIO_WAV:
        drwav_seek_to_first_pcm_frame((drwav*)music.ctxData);
        break;
#endif
#if defined(SUPPORT_FILEFORMAT_OGG)
    ...
    case MUSIC_AUDIO_MP3:
        drmp3_seek_to_start_of_stream((drmp3*)music.ctxData);
        break;
#endif
#if defined(SUPPORT_FILEFORMAT_QOA)
    case MUSIC_AUDIO_QOA:
        qoaplay_rewind((qoaplay_desc*)music.ctxData);
        break;
#endif
#if defined(SUPPORT_FILEFORMAT_FLAC)
    case MUSIC_AUDIO_FLAC:
        drflac__seek_to_first_frame((drflac*)music.ctxData);
        break;
#endif
#if defined(SUPPORT_FILEFORMAT_XM)
...
void UpdateMusicStream(Music music) {
    ...
    for (int i = 0; i < 2; i++) {
        if (!music.stream.buffer->isSubBufferProcessed[i])
        ...
        if ((framesLeft >= subBufferSizeInFrames) || music.looping)
        ...
        case MUSIC_AUDIO_WAV: {
            if (music.stream.sampleSize == 16) {
                while (true) {
                    int frameCountRead = (int)drwav_read_pcm_frames_s16((drwav*)music.ctxData, frameCountStillNeeded,
                                                                        (short*)((char*)AUDIO.System.pcmBuffer + frameCountReadTotal * frameSize));
                ...
                while (true) {
                    int frameCountRead = (int)drwav_read_pcm_frames_f32((drwav*)music.ctxData, frameCountStillNeeded,
                                                                        (float*)((char*)AUDIO.System.pcmBuffer + frameCountReadTotal * frameSize));
                    frameCountReadTotal += frameCountRead;
                    frameCountStillNeeded -= frameCountRead;
                    if (frameCountStillNeeded == 0)
                        break;
        ...
        case MUSIC_AUDIO_OGG: {
            while (true) {
                int frameCountRead = stb_vorbis_get_samples_short_interleaved(
                    (stb_vorbis*)music.ctxData, music.stream.channels, (short*)((char*)AUDIO.System.pcmBuffer + frameCountReadTotal * frameSize),
                    frameCountStillNeeded * music.stream.channels);
                frameCountReadTotal += frameCountRead;
                frameCountStillNeeded -= frameCountRead;
                if (frameCountStillNeeded == 0)
                    break;
        ...
        case MUSIC_AUDIO_MP3: {
            while (true) {
                int frameCountRead = (int)drmp3_read_pcm_frames_f32((drmp3*)music.ctxData, frameCountStillNeeded,
                                                                    (float*)((char*)AUDIO.System.pcmBuffer + frameCountReadTotal * frameSize));
                frameCountReadTotal += frameCountRead;
                frameCountStillNeeded -= frameCountRead;
                if (frameCountStillNeeded == 0)
        ...
        case MUSIC_AUDIO_QOA: {
            ...
            while (true)
            {
                int frameCountRead = (int)qoaplay_decode((qoaplay_desc *)music.ctxData, (float *)((char *)AUDIO.System.pcmBuffer +
            frameCountReadTotal*frameSize),  frameCountStillNeeded); frameCountReadTotal += frameCountRead; frameCountStillNeeded -= frameCountRead;
        ...
        case MUSIC_AUDIO_FLAC: {
            while (true) {
                int frameCountRead = (int)drflac_read_pcm_frames_s16((drflac*)music.ctxData, frameCountStillNeeded,
                                                                     (short*)((char*)AUDIO.System.pcmBuffer + frameCountReadTotal * frameSize));
                frameCountReadTotal += frameCountRead;
                frameCountStillNeeded -= frameCountRead;
                if (frameCountStillNeeded == 0)
        ...
        case MUSIC_MODULE_XM: {
            ...
            if (AUDIO_DEVICE_FORMAT == ma_format_f32)
                jar_xm_generate_samples((jar_xm_context_t*)music.ctxData, (float*)AUDIO.System.pcmBuffer, framesToStream);
            else if (AUDIO_DEVICE_FORMAT == ma_format_s16)
                jar_xm_generate_samples_16bit((jar_xm_context_t*)music.ctxData, (short*)AUDIO.System.pcmBuffer, framesToStream);
            else if (AUDIO_DEVICE_FORMAT == ma_format_u8)
                jar_xm_generate_samples_8bit((jar_xm_context_t*)music.ctxData, (char*)AUDIO.System.pcmBuffer, framesToStream);
            // jar_xm_reset((jar_xm_context_t *)music.ctxData);
...
void AttachAudioMixedProcessor(AudioCallback process) {
    ...
    if (last) {
        ...
        AUDIO.mixedProcessor = processor;
...
void DetachAudioMixedProcessor(AudioCallback process) {
    ...
    while (processor) {
        ...
        if (processor->process == process) {
            if (AUDIO.mixedProcessor == processor)
...
static ma_uint32 ReadAudioBufferFramesInInternalFormat(AudioBuffer* audioBuffer, void* framesOut, ma_uint32 frameCount) {
    ...
    while (1) {
        ...
        memcpy((unsigned char*)framesOut + (framesRead * frameSizeInBytes), audioBuffer->data + (audioBuffer->frameCursorPos * frameSizeInBytes),
...
static ma_uint32 ReadAudioBufferFramesInMixingFormat(AudioBuffer* audioBuffer, float* framesOut, ma_uint32 frameCount) {
    ...
    while (totalOutputFramesProcessed < frameCount) {
        ...
        ma_uint64 inputFramesProcessedThisIteration =
            ReadAudioBufferFramesInInternalFormat(audioBuffer, inputBuffer, (ma_uint32)inputFramesToProcessThisIteration); /* Safe cast. */
        ma_uint64 outputFramesProcessedThisIteration = outputFramesToProcessThisIteration;
        ma_data_converter_process_pcm_frames(&audioBuffer->converter, inputBuffer, &inputFramesProcessedThisIteration, runningFramesOut,
                                             &outputFramesProcessedThisIteration);
    ...
    if (stream.buffer != NULL) {
        if (stream.buffer->isSubBufferProcessed[0] || stream.buffer->isSubBufferProcessed[1]) {
            ...
            unsigned char* subBuffer =
                stream.buffer->data + ((subBufferSizeInFrames * stream.channels * (stream.sampleSize / 8)) * subBufferToUpdate);
